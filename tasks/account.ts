...
const main = async ({ save }: { save: boolean }, hre: HardhatRuntimeEnvironment) => {
...
`);
  ...
  if (save) {
    ...
    console.info(`✅ Saved the private key to '${filePath}' file.\n`);
  }
};
...
task("account", descTask, main)
  .addFlag("save", descSaveFlag)
